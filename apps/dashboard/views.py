...
class DashboardView(LoginRequiredMixin, AdministratorRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        stats = orders.aggregate(
            ...
            in_progress=Count("pk", filter=Q(stage__in=[Order.Stage.PRE_PRESS, Order.Stage.PRODUCTION])),
            ready=Count("pk", filter=Q(stage=Order.Stage.READY)),
            awaiting_payment=Count("pk", filter=Q(stage=Order.Stage.AWAITING_PAYMENT) | (~Q(payment_status=Order.PaymentStatus.PAID) & ~Q(stage__in=[Order.Stage.CANCELLED, Order.Stage.DELIVERED, "finalizado"]))),
            stage_new=Count("pk", filter=Q(stage=Order.Stage.NEW)),
            stage_preparation=Count("pk", filter=Q(stage__in=[Order.Stage.AWAITING_PAYMENT, Order.Stage.PAYMENT_CONFIRMED, Order.Stage.PRE_PRESS])),
        ...
        context.update({
            ...
            "late_orders": orders.filter(stage__in=[Order.Stage.NEW, Order.Stage.AWAITING_PAYMENT, Order.Stage.PAYMENT_CONFIRMED, Order.Stage.PRE_PRESS, Order.Stage.PRODUCTION]).filter(Q(due_at__lt=now) | Q(stage_updated_at__lt=idle_cutoff)).order_by("due_at", "stage_updated_at")[:8],
