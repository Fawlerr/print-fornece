...
def apply_calculator_receipt_schema(apps, schema_editor):
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            ...
            "WHERE payment_status = %s AND (receipt_client_name IS NULL OR receipt_client_name = '')",
            ["pago"],
        )
        cursor.execute(
            f"UPDATE {order_table} SET receipt_total_amount = total_amount "
            "WHERE payment_status = %s AND receipt_total_amount IS NULL",
            ["pago"],
        )
        cursor.execute(
            f"UPDATE {order_table} SET receipt_paid_amount = paid_amount "
            "WHERE payment_status = %s AND receipt_paid_amount IS NULL",
            ["pago"],
        )
        cursor.execute(
            f"UPDATE {order_table} SET receipt_payment_method = COALESCE(payment_method, '') "
            "WHERE payment_status = %s AND (receipt_payment_method IS NULL OR receipt_payment_method = '')",
            ["pago"],
        )
